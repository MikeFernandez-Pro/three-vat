// three-vat's merged-materials example, on WebGL (ADR-0028): the robot crowd,
...
//
// Hold this file next to webgpu/merged.ts: the sections are the same, and the
// VAT section differs from the crowd page's by one bake option.
import * as THREE from "three";
import { bakeVAT } from "three-vat";
import type { VAT, VATClip, VATClock } from "three-vat";
...
createDemoGUI(params, stage, { setCount }, {
  ...
  addControls(gui) {
    ...
    gui
      ...
      .onChange((v: boolean) => setMerged(v));
  },
});
