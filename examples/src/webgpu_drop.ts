// The WebGPU drop page's door: ask the browser for an adapter, then either
...
if (support.ok) {
  await import("./webgpu/drop.js");
} else {
  const notice = document.getElementById("unsupported")!;
  notice.querySelector("#reason")!.textContent = support.reason;
  notice.hidden = false;
  // The HUD goes with it. Its readouts are measurements of a bake and the
  // crowd drawn from it, and there is neither: a HUD left on screen would be
  // stating figures for an example that never ran.
