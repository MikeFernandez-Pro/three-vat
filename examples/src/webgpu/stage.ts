// Everything around the crowd on the WebGPU path: renderer, camera, lights,
// ground, environment and fog. A deliberate near-copy of webgl/stage.ts
// (ADR-0011) — the differences below are the renderers' differences, and a
// shared harness would have hidden every one of them:
//
//   * the whole module imports `three/webgpu`, not `three`;
//   * `createStage` is async, because a `WebGPURenderer` has no device — and so
//     no max texture size to bake against — until `await renderer.init()`;
//   * the ground is a `MeshStandardNodeMaterial`, the node system's own;
...
function gradientEnv(top: string, bottom: string): THREE.Scene {
  ...
  env.add(
    new THREE.Mesh(
      ...
      new THREE.MeshBasicNodeMaterial({ map: texture, side: THREE.BackSide }),
    ),
  );
  return env;
}
 ...
 *
 * Async where the WebGL stage is not: the backend is created in `init()`, and
 * everything after it here — the PMREM presets, and the page's own
 * `getMaxTextureSize` for the bake — needs a live device.
 */
export async function createStage(params: DemoParams): Promise<Stage> {
  ...
  // The device, before anything asks it a question.
  await renderer.init();
  ...
  const stage: Stage = {
     ...
     *
     * `shadowMap.enabled` is part of what each material's pipeline is built
     * for, here as on WebGL, so every material has to be flagged for rebuild or
     * the flip is silently ignored.
     */
    applyShadows() {
