// three-vat's worker example, on WebGPU (ADR-0026): the same bake, run in a Web
...
//
// Hold this file next to webgl_worker.ts. Nothing shared forces the
...
// are the renderers', not the library's. The bake itself touches no renderer,
// and the worker is the one file both pages share.
//
// Loaded by webgpu_worker.ts only once WebGPU is known to work, so a browser
// without it never fetches the node-material bundle.
import * as THREE from "three/webgpu";
import { uniform } from "three/tsl";
import { bakeVAT, bakeVATInWorker } from "three-vat";
import type { DeltaVAT, VATClip } from "three-vat";
