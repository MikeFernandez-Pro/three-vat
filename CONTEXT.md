...
The two VAT layers, and they are not the same kind of texture. The position layer is `RGBA16F` — a delta-encoded position in eight bytes a texel, half-float because a *delta* loses a fixed fraction of itself to it (0.061%) and nothing at the rest pose, and because the one thing half-float cannot hold — a component past 65 504 — the baker refuses by name. The normal layer is `RG8` — an absolute normal, **octahedral**, in two. Lighting is visibly wrong with positions alone, so the normal layer is baked by default. `bakeNormals: false` drops it — 2 B of the 10 B a vertex-frame costs — for the two setups that genuinely do not read it: an unlit material, and `flatShading: true`, where three derives a better normal from the deformed position. Any other shading material paired with such a VAT is refused, not rendered.

**Octahedral**:
How a baked normal is stored: the octahedron `|x| + |y| + |z| = 1` projected to a square, its far hemisphere folded into the corners, quantised to two unsigned bytes. A normal is a direction, so what the encoding costs is an **angle** — ~0.95° worst case, ~0.33° mean — and error in a normal is shading, never geometry (ADR-0002's amendment, #29). Defined once, on the CPU, in `src/octahedral.ts`: the baker and the **texture panel** call it; the two shader decodes are transcriptions of it, which is the seam the **parity gate** is there to hold.
...
**Fallback**:
What a default (`'auto'`) bake does when the rig encoding refuses the asset and the vertex encoding does not: it bakes the vertex encoding instead, and records the rig refusal's message on the VAT as `vat.fallback` (ADR-0027, ADR-0029). It prints nothing, because an asset whose clips animate morphs is ordinary and the vertex encoding is right for it. `vat.fallback` is `null` on a vertex-encoded VAT asked for by name. A bake whose vertex encoding refuses as well throws, and carries the rig refusal as its `cause`. A refusal both encodings share never falls back.
...
**Pack**:
The fixed-size layout instance playback is carried in: five RGBA-shaped slots — clip, playback, crossfade, outgoing clip, outgoing playback — rather than one field per slot. RGBA-shaped because the layout is meant to outlive the thing carrying it, which it did: it was three instanced `vec4`s when ADR-0009 shaped it, three texels of the **playback texture** from 2.0 (ADR-0016), and five once the **crossfade** made the band an instance is leaving a second live playback state (ADR-0025). The order is part of the contract: the duration sits third, so the three texels 2.0 laid out keep their places. Both decodes read all five every frame, transitioning or not, because a branch around the outgoing pair cost an idle crowd more than the fetches (#72). Published 1.x never saw any of it — that release carried five one-float attributes. The pack is the layout — it names that, never the values in it, and never what holds them.
