...
- **The TSL decode sampled normals per fragment, with an interpolated vertex index.** `normalNode` is built in the *fragment* stage — three reaches it from `normalView` through `builder.context.setupNormal()` — and `IndexNode` outside the vertex stage does not hand back the vertex index: it turns itself into a varying. Every fragment was therefore reading a linearly *interpolated* index, which addresses neither of the vertices it sits between, so the normal for most of every triangle was fetched from an unrelated vertex. Because `vertexIndex` is one shared immutable node, the position decode resolved through the same cached varying and moved with it — which is why a WebGPU crowd had parts drifting and scaling independently of each other while the same VAT rendered correctly on the WebGL path. The sample is now wrapped in `varying()`, which forces it back into the vertex stage and interpolates the normal *vector* rather than the index — exactly what the GLSL path does, writing `objectNormal` in `beginnormal_vertex` and letting the rasteriser interpolate the result; normalising happens after interpolation on both paths, because interpolating unit vectors does not preserve their length. Found by the new cross-path pixel-diff gate, which is the only check in this repository that could have found it: four structural test files asserted the node graph and said nothing.
