...
**Since [#29](https://github.com/MikeFernandez-Pro/three-vat/issues/29) and
[#73](https://github.com/MikeFernandez-Pro/three-vat/issues/73) the `texture`
row's left-hand figure is 7.9 MB, not 25.2.** The table is left as it was
measured, because it is a bench and the bench ran against a bake that was RGBA
float on both layers. What changed is the encoding, not the measurement: a
vertex-encoded texel pair went from 32 B to 18 B and then to 10 B ([ADR-0002's
amendments](./0002-runtime-texture-encoding.md)), so the same Soldier bake is
now 7.9 MB and the memory ratio is ~45× rather than ~140×. Both frame rows
stand — a phone's cache still misses on 8 MB, where it holds the 177 kB rig
