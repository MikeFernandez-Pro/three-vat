...
- **Only the tracks the bake reads cross.** *Amended by [#89](https://github.com/MikeFernandez-Pro/three-vat/issues/89):* a track on anything but a node's transform, a mesh's morphs or a bone stays on the page. The worker's materials and non-mesh nodes are bare stand-ins, so a material colour or light intensity track would find nothing there to bind, and three would log an error the page's bake does not. No frame reads such a track, so dropping it changes no texel. A dropped track is not refused either, whatever its interpolant.
