...
- **The batched examples bake with it** (#83). A `BatchedMesh` takes one material, so `webgl_batched.html` and `webgpu_batched.html` drew every part of the robot in its first material's grey. Merged, the one material they clone carries all three colours in the vertices, and each page's draw count is what it was: the batch still holds one geometry and one material, which is all [ADR-0023](./0023-a-one-geometry-batch-is-one-draw-on-webgpu-in-the-example-not-the-library.md)'s fold asks of it.
