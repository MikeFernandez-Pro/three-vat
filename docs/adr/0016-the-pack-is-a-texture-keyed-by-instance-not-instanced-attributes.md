...
- **The demo bench (340 robots) may not lose more than 5% of its frame rate.** The three pack fetches are per-vertex but read the same texel for every vertex of an instance, so the texture cache should absorb them; if it does not, the premise was wrong.
  ...
  > **Held, and re-measured once since.** The premise was right about the pack: [#72](https://github.com/MikeFernandez-Pro/three-vat/issues/72) widened the row to five texels for the crossfade and measured no cost at all for the width. It found 10% elsewhere — in the `if` the GLSL decode had grown around the crossfade, which an idle crowd paid for in occupancy without ever taking it — and that branch is gone on the vertex encoding, which puts the idle crowd back on 0.283 ms against 0.282 ms before. See [0025's measurement](./0025-the-crossfade-is-a-second-live-band-in-the-pack.md#the-measurement-the-branch-is-the-one-that-costs). The bound is worth the sentence it cost: nothing about the crossfade's shape looked expensive, and the reasoning that said so was wrong in both directions.
