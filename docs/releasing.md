...
5. **`node release/drop/check.mjs` passes if the drop example changed.** It
   ...
   and a file that will not parse, drops Soldier as a `.gltf` with its `.bin`
   and textures, picks it as a folder, drops it missing a texture, drops a
   Draco and a meshopt + KTX2 `.glb`, drops `Samba Dancing.fbx`, finds its
