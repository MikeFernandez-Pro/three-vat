...
5. **`node release/drop/check.mjs` passes if the drop example changed.** It
   ...
   the button, forces the vertex encoding and puts it back, drops an `.obj`
   and a file that will not parse, drops `Samba Dancing.fbx`, finds its empty
   `Take 001` unchecked, turns its merge off and unchecks its last clip, reads
   the HUD after each, and fails on any console error. It needs the FBX that
...
| Both paths were handed the same bake, the same spanned bake, and the same rig bake | Each path bakes its own VAT (a texture cannot be shared between two live renderers without asking a question the gate is not about), so the guarantee that both saw identical texels is taken back as evidence — a texel walk over both layers of each vertex bake, and over the rig texture of the rig bake. The spanned bake is also checked to *span*: two rows a frame, or its checks below would compare the one-row layout with itself. |
| Both paths drew something | Two empty frames match perfectly. Without this, a harness that never built a crowd reports the cleanest pass of its life. Checked for the robot, the spanned robot, the batched crowd and the rig crowd alike. |
...
| The two decode paths render the same pixels | The gate. The crowd it compares is caught **mid-transition**: one of the three instances is blending out of a second clip that is still playing, at a weight of very nearly a half at `TIME` ([ADR-0025](./adr/0025-the-crossfade-is-a-second-live-band-in-the-pack.md)). A crossfade is a third decode on each path — two bands resolved and mixed — and the weight between them is the half each path computes for itself, so a crowd that never transitioned would compare the other two decodes and call it parity. It comes out of the one instance table, so the batched frames and the rig case render it too; where in the transition `TIME` falls is asserted in `scene.test.ts` through `resolveVATFrame`, not eyeballed here. |
| The two decode paths agree on a crowd whose frames span rows | The robot baked again at a phone's `maxTextureSize` of 4096, so each frame's 7 214 vertices span two rows of 3 607 ([ADR-0030](./adr/0030-a-vertex-encoded-frame-spans-rows-past-the-ceiling.md)). The two paths spell the arithmetic apart, a literal modulo and divide in GLSL and integer nodes in TSL, so it is compared like any decode. |
| Each path draws the spanned bake as it draws the one-row bake (measured within that path) | The spanned bake holds the one-row bake's texels in other places, so a correct decode draws the same crowd from either, pixel for pixel. This is the check that catches a stride both paths got wrong the same way, which the one above would call parity. Breaking the GLSL row offset for one run failed it on that path alone, at 67% of the drawn pixels. |
