...
export class Frustum {
    ...
    setFromProjectionMatrix(mat) {
        ...
        if (this.coordinateSystem === WebGLCoordinateSystem) {
            ...
            throw new Error('Invalid coordinate system: ' + this.coordinateSystem);
        return this;
    }
    updatePlane(index, x, y, z, constant) {
    ...
    intersectsBoxMask(box, mask) {
        const array = this.array;
        let xMin, yMin, zMin, xMax, yMax, zMax;
        for (let i = 0; i < 6; i++) {
            if ((mask & (0b100000 >> i)) === 0)
                continue; // if bit i is 0
            const offset = i * 4;
    ...
    isIntersectedMargin(box, mask, margin) {
        if (mask === 0)
            return true;
        const array = this.array;
        for (let i = 0; i < 6; i++) {
