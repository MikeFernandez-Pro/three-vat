...
export function intersectRayBox(box, origins, dirsInv, signs, near, far) {
    // X axis
    let sign = signs[0];
...
export function intersectBoxBox(A, B) {
    return A[1] >= B[0] && B[1] >= A[0] && A[3] >= B[2] && B[3] >= A[2] && A[5] >= B[4] && B[5] >= A[4];
}
export function intersectSphereBox(center, radius, box) {
    return minDistanceSqPointToBox(box, center) <= radius * radius;
}
//# sourceMappingURL=intersectUtils.js.map