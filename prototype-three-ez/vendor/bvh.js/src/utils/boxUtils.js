export function unionBox(A, B, target) {
...
export function unionBoxChanged(A, B, target) {
    let changed = false;
...
export function isBoxInsideBox(innerBox, outerBox) {
...
export function isExpanded(A, target) {
    let expanded = false;
    if (target[0] > A[0]) {
        target[0] = A[0];
        expanded = true;
...
export function expandBox(A, target) {
    if (target[0] > A[0])
        target[0] = A[0];
    if (target[1] < A[1])
        target[1] = A[1];
...
export function expandBoxByMargin(target, margin) {
    target[0] -= margin;
    target[1] += margin;
    target[2] -= margin;
    target[3] += margin;
...
export function getLongestAxis(box) {
...
export function minDistanceSqPointToBox(box, point) {
...
export function minDistancePointToBox(box, point) {
    return Math.sqrt(minDistanceSqPointToBox(box, point));
}
export function minMaxDistanceSqPointToBox(box, point) {
...
export function minMaxDistancePointToBox(box, point) {
