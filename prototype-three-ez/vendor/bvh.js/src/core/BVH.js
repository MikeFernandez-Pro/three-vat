...
import { Frustum, WebGLCoordinateSystem } from '../utils/frustum.js';
import { intersectBoxBox, intersectRayBox, intersectSphereBox } from '../utils/intersectUtils.js';
export class BVH {
    get root() {
        return this.builder.root;
    }
    constructor(builder, coordinateSystem = WebGLCoordinateSystem) {
        this._sign = new Uint8Array(3);
    ...
    createFromArray(objects, boxes, onLeafCreation, margin) {
        if (objects?.length > 0) {
    ...
    intersectsRay(dir, origin, onIntersection, near = 0, far = Infinity) {
        if (this.root === null)
            return false;
        ...
        function _intersectsRay(node) {
            if (!intersectRayBox(node.box, origin, dirInv, sign, near, far))
                return false;
            if (node.object !== undefined)
                return onIntersection(node.object);
            return _intersectsRay(node.left) || _intersectsRay(node.right);
        }
    }
    intersectsBox(box, onIntersection) {
        if (this.root === null)
            return false;
        return _intersectsBox(this.root);
        function _intersectsBox(node) {
            if (!intersectBoxBox(box, node.box))
                return false;
            if (node.object !== undefined)
                return onIntersection(node.object);
    ...
    intersectsSphere(center, radius, onIntersection) {
        if (this.root === null)
            return false;
        return _intersectsSphere(this.root);
        function _intersectsSphere(node) {
            if (!intersectSphereBox(center, radius, node.box))
                return false;
            if (node.object !== undefined)
                return onIntersection(node.object);
    ...
    isNodeIntersected(node, onIntersection) {
        const nodeBox = node.box;
        let parent;
        while ((parent = node.parent)) {
            const oppositeNode = parent.left === node ? parent.right : parent.left;
            if (_isNodeIntersected(oppositeNode))
                return true;
            node = parent;
        }
        return false;
        function _isNodeIntersected(node) {
            if (!intersectBoxBox(nodeBox, node.box))
                return false;
            if (node.object !== undefined)
                return onIntersection(node.object);
    ...
    rayIntersections(dir, origin, onIntersection, near = 0, far = Infinity) {
        if (this.root === null)
            return;
        const dirInv = this._dirInv;
        const sign = this._sign;
        ...
        function _rayIntersections(node) {
            if (!intersectRayBox(node.box, origin, dirInv, sign, near, far))
                return;
            if (node.object !== undefined) {
    ...
    frustumCullingLOD(projectionMatrix, cameraPosition, levels, onIntersection) {
        if (this.root === null)
            return;
        const frustum = this.frustum.setFromProjectionMatrix(projectionMatrix);
        _frustumCullingLOD(this.root, 0b111111, null);
        function _frustumCullingLOD(node, mask, level) {
            const nodeBox = node.box;
    ...
    closestPointToPoint(point, onClosestDistance) {
        if (this.root === null)
            return;
